...
pub fn I() -> impl IntoView {
    ...
    let drag = move |start: usize, end: usize| {
        ...
        let link_v = link_vec.clone().get().clone();
        let mut nav_title_one = link_v;
        let item_save = nav_title_one.remove(start);
        nav_title_one.insert(end, item_save);

        spawn_local(async move {
            let res = crate::link_sort(nav_title_one, crate::get_global_token_with_bearer()).await;
            match res {
                Ok(res) => {
    ...
    view! {
        ...
        <myw::Gap/>
        <Table data=link_vec col_vec=col_vec on_row_drop=Box::new(move |(start, end)|{
            drag(start, end);
        })> </Table>
